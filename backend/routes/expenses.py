...
from models.expense import Budget
...
def _get_user_id(source: dict, field="user_id"):
    """
...
def _parse_datetime(value, field_name):
    """Parse ISO string or datetime. Returns (datetime, None) or (None, error_str)."""
    if value in (None, ""):
        return None, None
    ...
    if isinstance(value, str):
        for fmt in ("%Y-%m-%d", None):
            try:
                if fmt:
                    return datetime.strptime(value.strip(), fmt).replace(tzinfo=timezone.utc), None
                else:
...
def _parse_items_json(items_data):
    """Parse items from a JSON list. Returns ([ExpenseItem, ...], None) or (None, error_str)."""
    ...
    for item_data in items_data:
        ...
        item = ExpenseItem(name=str(name), quantity=item_data.get("quantity", 1), price=item_data.get("price", 0))
...
def _parse_items_multipart(form):
    raw = parse_items_from_form(form)
    items = []
    for r in raw:
        item = ExpenseItem(name=r["name"], quantity=r.get("quantity", 1), price=float(r["price"]))
        is_valid, error = item.validate()
        if not is_valid:
            return [], error  # ← fix here
        items.append(item)
    return items, None
...
def get_expenses():
    ...
    try:
        ...
        return jsonify({"error": "page and per_page must be valid integers (1-100)"}), 400
    ...
    cursor = col.find(query).sort("date", -1).skip((page - 1) * per_page).limit(per_page)
...
def create_expense():
    ...
    # right after is_multipart line
    print(">>> content_type:", request.content_type)
    print(">>> is_multipart:", is_multipart)
    print(">>> request.files:", request.files)
    print(">>> receipt file:", request.files.get("receipt"))
    ...
    # ── Handle receipt upload FIRST before anything else ─────────────────
    receipt_image = None
    if is_multipart:
        uploaded_file = request.files.get("receipt")
        if uploaded_file and uploaded_file.filename:
            upload_result = save_receipt_image(uploaded_file, app=current_app._get_current_object())
            if not upload_result["success"]:
                return jsonify({"error": upload_result["error"], "field": "receipt"}), 400
            receipt_image = upload_result["filepath"]
    # right after the upload block
    print(">>> receipt_image after upload:", receipt_image)
    # ── Parse items ───────────────────────────────────────────────────────
    items = []
    if is_multipart:
...
def update_expense(expense_id):
    ...
    col = _get_expenses_collection()
    existing_data = col.find_one({"_id": ObjectId(expense_id), "user_id": user_id})
    ...
    if is_multipart:
        ...
        if uploaded_file and uploaded_file.filename:
            ...
            if existing_data.get("receipt_image"):
                delete_receipt_image(existing_data["receipt_image"], app=current_app._get_current_object())
    ...
    refreshed = col.find_one({"_id": ObjectId(expense_id), "user_id": user_id})
    return jsonify({
...
def delete_expense(expense_id):
    ...
    col = _get_expenses_collection()
    existing = col.find_one({"_id": ObjectId(expense_id), "user_id": user_id})
    if not existing:
        return jsonify({"error": "Expense not found"}), 404

    if existing.get("receipt_image"):
        delete_receipt_image(existing["receipt_image"], app=current_app._get_current_object())
...
def create_budget():
    ...
    try:
        ...
        return jsonify({"error": "Server error. Could not save budget."}), 500